...
import pytest
from utils.checks import get_endpoint_response, check_delete_requests, get_response_from_request_button
import json
from fixtures import browser, test_data, get_api


@pytest.mark.usefixtures('browser', 'test_data', 'get_api')
def test_delete_web(browser, test_data, get_api):
    """
    ...
    assert test_data['response'] == ep_response['text'], f"Expected {test_data['response']}," \
                                                         f"but got {ep_response['text']}"

    req_button = get_response_from_request_button(browser)
    api_response = check_delete_requests(test_data, get_api)
    assert json.loads(req_button) == api_response.text, f'Response from API: {json.loads(api_response.text)},' \
